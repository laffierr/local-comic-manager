...
- `versionCode` 递增 1 (当前 20)
- `versionName` 按语义化版本递增 (当前 0.6.5)
...
| 加新功能（兼容旧版）且当前版本已稳定 | MINOR | 0.6.0 → 0.7.0 |
| 破坏性改动（不兼容旧版） | MAJOR | 0.6.0 → 1.0.0 |

**重要：MINOR 版本晋升规则：** 只有确定当前版本没有明显 bug、达到稳定状态时，才推进 MINOR 版本（中间数字）。如果始终有 bug 在修，则持续推进 PATCH 版本（最后一位），例如 0.6.4 → 0.6.5 → ... → 0.6.20，直到稳定后再跳到 0.7.0。
